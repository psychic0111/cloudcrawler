...
<script src="${pageContext.request.contextPath}/js/dhtmlxTree/codebase/dhtmlxtree.js"></script>
	...
	function addFrm_submit(){
		...
		AT.postFrm("addTemplateForm",function(data){
			...
			var params = $("#searchTemplate").serialize();
			var url = "${pageContext.request.contextPath}/template/templateList.do?pageNo="+pageNo+"&pageSize="+pageSize+"&"+params+"&num="+Math.random();
			$("#treeAndList").load(url,iframeHeight);	//刷新内容列表
			closeDg();
		},true);
	}
		...
		if($checked.length==0){
			...
			MSG.confirm('确认要删除选中的模板吗？',null,function(){
				AT.post("${pageContext.request.contextPath}/template/del.do",$checked.serialize(),function(data){
					...
					var params = $("#searchTemplate").serialize();
					var url = "${pageContext.request.contextPath}/template/templateList.do?pageNo="+pageNo+"&pageSize="+pageSize+"&"+params+"&num="+Math.random();
					AT.load("treeAndList",url,iframeHeight);
				});
			},null);
...
function doPage(ths,pageNo,pageSize){
	AT.load("iframepage","${pageContext.request.contextPath}/template/templateContentList.do?pageNo="+pageNo+"&pageSize="+pageSize+"&categoryId="+$("#fristLevel").val()+"&name="+encodeURIComponent($("#template_name").val())+"&isRoot=0",iframeHeight);
}
