	...
	<form id="searchTemplate" class="indexForm"  action="${pageContext.request.contextPath}/template/templateContentList.do" method="post">
       ...
       <div class="right">
               	<span style="display:inline;">所属分类：</span>
        			...
        			<span style="display:inline;">模板名称：</span><input id="template_name" type="text" name="name" style="width: 100px;display:inline;" value="${template.name}" class="inputTxt1" maxlength="100" />
         ...
         <c:forEach items="${list}" var="template" varStatus="number" >
         	<tr>
         		...
         		<td class="TD_Bgcolor1" >	
         			<div align="left">
	         			<c:choose>
	       				<c:when test="${fn:length(template.urlRule) >30}">
	       					<c:out value="${fn:substring(template.urlRule, 0 ,30)}..."></c:out>
	       				</c:when>
	         			<c:otherwise>
