    		...
    		<c:if test="${crawler.status == 1}">
    			<c:choose>  
				  <c:when test="${crawler.crawlerStatus == 1}">  
   					<input id="dataStatusA_${crawler.id}" type="button" class="botton_style3"  value="启动" onclick="startCrawlerControl('${crawler.code}');" />
 				  </c:when>
 				  <c:when test="${crawler.crawlerStatus == 2}">  
   					<input id="dataStatusB_${crawler.id}" type="button" class="botton_style2"  value="停止" onclick="endCrawlerControl('${crawler.code}');"  />
 				  </c:when>
				</c:choose>  
			</c:if>
