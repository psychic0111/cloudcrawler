...
/**
 * Token with encapsulation of LexerState during parse. Models view at token
 * text by removing syntactic sugar from tokens with contains it, e.g. STRING,
 * URI, FUNCTION
 * 
 * @author kapy
 * 
...
public class CSSToken extends CommonToken {
	 ...
	 *            Type of token
	 * @param state
	 *            State of lexer, which will be copied
	 */
	...
	/**
	 * Creates CSSToken of given type with cloning lexer state automatically,
	 * allows to set text boundaries in input stream
	 * 
	 * @param type
	 *            Type of token
	 * @param state
	...
	/**
	 * Considers text as content of STRING token, and models view at this text
	 * as an common string, that is one character removed from the both
	 * beginning and the end.
	 * 
	 * @param string
	 *            Content of STRING token
	...
	/**
	 * Considers text as content of URI token, and models view at this text as
	 * an common string, that is removed {@code 'url('} from the beginning and
	 * {@code ')'} from the and. If result of this operation is STRING, remove
	 * even quotation marks
	 * 
	 * @param uri
	 *            Content of URI token
	 * @return String with trimmed URI syntax sugar and optionally quotation
	 *         marks
	 */
	public static String extractURI(String uri) {
		String ret = uri.substring(4, uri.length() - 1);
		// trim string
		if (ret.length() > 2 && (ret.charAt(0) == '\'' || ret.charAt(0) == '"')) {
	...
	/**
	 * Considers text as content of FUNCTION token, and models view at this text
	 * as an common string, that is removed {@code '('} from the end of string
	 * 
	 * @param function
	 *            Content of FUNCTION token
	...
	/**
	 * Considers text as content of HASH token, and models view at this text as
	 * an common string, that is removed {@code '#'} from the beginning of
	 * string
	 * 
	 * @param hash
	 *            Content of HASH token
	...
	/**
	 * Considers text as content of CLASSKEYWORD token, and models view at this
	 * text as an common string, that is removed {@code '.'} from the beginning
	 * of string
	 * 
	 * @param className
	 *            Content of CLASSKEYWORD token
	...
	public String toString() {
		...
		sb.append("/").append(ls).append("/").append(super.toString());
