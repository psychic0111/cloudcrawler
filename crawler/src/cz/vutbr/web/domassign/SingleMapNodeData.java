...
/**
 * Implementation of NodeData by single HashMap. Is more
 * space efficient at the cost of speed.
 * 
 * @author kapy
 */
public class SingleMapNodeData implements NodeData {
	...
	public <T extends CSSProperty> T getProperty(String name) {
		// until java 7 compiler is not able to infer
		// correct type
	...
	public <T extends CSSProperty> T getProperty(String name, boolean includeInherited) {
		...
		// if there is no inferred type, cast to CSSProperty
		// is safe
		// otherwise the possibility having wrong left side
		// of assignment
		// is roughly the same as use wrong dynamic class
		// cast
	...
	public NodeData push(Declaration d) {
		...
		Map<String, CSSProperty> properties = new HashMap<String, CSSProperty>(COMMON_DECLARATION_SIZE);
		Map<String, Term<?>> terms = new HashMap<String, Term<?>>(COMMON_DECLARATION_SIZE);
	...
	public NodeData inheritFrom(NodeData parent) throws ClassCastException {
		...
		for (String key : nd.map.keySet()) {
			...
			// create new quadruple if this do not contain
			// one
			// for this property
			if (q == null)
				q = new Quadruple();
			...
			// insert/replace only if contains
			// inherited/original
			// value
			if (!q.isEmpty())
				map.put(key, q);
